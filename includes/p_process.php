...
if (isset($_POST['add'])) {
	...
	$db->connect();
	if (isset($tb)) {
		$db->execute("INSERT INTO tb_buku (judul, kodePenerbit, kodePengarang, tahun, edisi, issn_isbn, seri, abstraksi, kodeKategori, tglInput, tglUpdate, lastUpdateBy) VALUES
				('$judul','$kodePenerbit','$kodePengarang','$tahun','$edisi','$issn_isbn','$seri','$abstraksi','$kodeKategori','$date','$date','$kodePetugas')");
		redirect("?page=daftar_buku", "");
	} else if (isset($tk)) {
		$db->execute("INSERT INTO tb_kategori (namaKategori) VALUES ('$namaKategori')");
		redirect("?page=daftar_kategori", "");
	}
	if (isset($tpn)) {
		$db->execute("INSERT INTO tb_penerbit (nama, alamat, telp, email) VALUES ('$nama', '$alamat', '$telp', '$email')");
		redirect("?page=daftar_penerbit", "");
	}
	if (isset($tpg)) {
		$db->execute("INSERT INTO tb_pengarang (nama) VALUES ('$nama')");
		redirect("?page=daftar_pengarang", "");
	}
	if (isset($tup)) {
		$db->execute("INSERT INTO tb_petugas (username, password, nama, email, dateInput, dateUpdate, tempatLahir, tanggalLahir, alamat)
				VALUES ('$username','" . md5($password) . "','$nama','$email','$date','$date','tpl','$ttl','$alamat')");
		redirect("?page=daftar_petugas", "");
	}
	if (isset($tud)) {
		$db->execute("INSERT INTO tb_dosen (username, password, nama, email, dateInput, dateUpdate, tempatLahir, tanggalLahir, alamat)
				VALUES ('$username','" . md5($password) . "','$nama','$email','$date','$date','tpl','$ttl','$alamat')");
		redirect("?page=daftar_dosen", "");
	}
	if (isset($tum)) {
		$db->execute("INSERT INTO tb_mahasiswa (username, password, nama, email, dateInput, dateUpdate, tempatLahir, tanggalLahir, alamat)
				VALUES ('$username','" . md5($password) . "','$nama','$email','$date','$date','tpl','$ttl','$alamat')");
		redirect("?page=daftar_mahasiswa", "");
	}
}
